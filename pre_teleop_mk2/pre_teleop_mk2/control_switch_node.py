...
class ControlSwitchNode(Node):
    def __init__(self) -> None:
        ...
        # ========== VARIABLES ==========
        # switch ttc thresholds
        self.ttc_switch_to_mppi = 1
        self.ttc_switch_to_teleop = 0.8
        self.safe_confirm_time_s = 0.05

        # steering takeover grace
        ...
        # danger persistence (debounce mppi trigger)
    ...
    def _tick(self) -> None:
        ...
        # force reverse for recovery
        teleop_reverse = (
            teleop_fresh
            and self.last_teleop_cmd is not None
            and self.last_teleop_cmd.drive.speed < 0.0
        )

        # stale = unsafe (conservative)
        ...
        danger_raw = (ttc <= ttc_mppi_eff)
        safe = pred_fresh and (ttc >= self.ttc_switch_to_teleop)
        ...
        # ========== FULL AUTONOMOUS MODE ==========
        if (not teleop_reverse) and self.teleop_mppi_enable and mppi_fresh:
            self.mode = MODE_MPPI
            self.safe_since_time = None
        ...
        if self.mode == MODE_TELEOP:
            ...
            if safe and not self.teleop_mppi_enable:
        ...
        # NOTE: DEBUG LOGS: Gets mode/state
        ...
        # ========== COMMAND PASS THROUGH (with clamps) ==========
        ...
        if speed < 0.0:
            speed = self._clamp(speed, -self.max_reverse_speed, 0.0)
        else:
    ...
    def _mppi_enable_cb(self, msg: Bool) -> None:
        """Store fully autonomous mode activation status"""
