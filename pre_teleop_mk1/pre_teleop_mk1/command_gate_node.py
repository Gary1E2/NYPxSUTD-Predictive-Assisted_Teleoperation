...
class CommandGateNode(Node):
    def __init__(self):
        ...
        # fraction of rays that must be near to be considered "NOT empty"
        self.min_close_fraction = 0.02

        # empty check before allowing motion
        ...
        self.get_logger().info(f"command_gate_node active. scan_topic=/scan_filtered, near_dist_m={self.near_dist_m}")
    ...
    def _tick(self):
        ...
        # release estop/escape stop (only if surroundings is no longer empty for race track conditions)
        ...
        allow_motion = (
            ...
            and mppi_alive
            and cmd_fresh
            # and (not blocked_by_escape)      # escape stop active?
        )
